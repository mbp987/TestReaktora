...
public class AdminController {
	...
	void actionSaveNew(MouseEvent event) throws SQLException {
		...
		while (true) {
			if (tf_login.getText() != null) {
				...
				try {
					...
					if (rs.getString(1) != null) {
						...
						view_error.setContentText("Taki login już istnieje w bazie - podaj inny login");
						view_error.setHeaderText("Błąd!");
						view_error.setTitle("Okno błędu");
			...
			if (tf_haslo.getText() != null) {
				...
				Alert view_error = new Alert(AlertType.ERROR);
				view_error.setContentText("Nie wpisano hasła");
				view_error.setHeaderText("Błąd!");
				view_error.setTitle("Okno błędu");
				view_error.showAndWait();
				break;
	...
	void actionFilter(MouseEvent event) throws SQLException {
		...
		// Budowa zapytania do filtrowania użytkowników:
	...
	// Metody dotyczące sekcji pytań:
	// Metoda pokazująca wszystkie pytania
	@FXML
	...
	// Metoda uruchamiająca panel dodawania pytań
	@FXML
	...
	// Metoda zapisująca pytanie do bazy
	@FXML
	void actionQuestionSave(ActionEvent event) throws SQLException {
		...
		while (true) {
			if (combo_questionLang.getValue() != null) {
				...
				Alert view_error = new Alert(AlertType.ERROR);
				view_error.setContentText("Nie wpisano języka");
				view_error.setHeaderText("Błąd!");
				view_error.setTitle("Okno błędu");
				view_error.showAndWait();
				break;
			...
			if (ta_questionText.getText() != null) {
				...
				Alert view_error = new Alert(AlertType.ERROR);
				view_error.setContentText("Nie wpisano treści pytania");
				view_error.setHeaderText("Błąd!");
				view_error.setTitle("Okno błędu");
				view_error.showAndWait();
				break;
			...
			// Pobranie prawidłowej odpowiedzi z radio buttonów
			{
				if (!rb_answear1.equals("")) {
					...
					Alert view_error = new Alert(AlertType.ERROR);
					view_error.setContentText("Nie wybrałeś żadnej prawidłowej odpowiedzi");
					view_error.setHeaderText("Błąd!");
					view_error.setTitle("Okno błędu");
					view_error.showAndWait();
					break;
	...
	// Metoda uruchamiająca panel edycji pytania zaznaczonego w tabeli
	@FXML
	void actionEditQuestion(ActionEvent event) {
		...
		if (selectedItem != null) {
			...
			rb_answearFalse.setSelected(true);
			// Znalezienie prawidłowej odpowiedzi i ustawienie radio button'a
			{
	...
	// Metoda usuwająca zaznaczone w tabeli pytanie
	@FXML
	void actionDeleteQuestion(ActionEvent event) throws SQLException {
		...
		if (selectedItem != null) {
			...
			Alert view_error = new Alert(AlertType.ERROR);
			view_error.setContentText("Nie wybrano żadnego wiersza w tabeli do usunięcia - wybierz wiersz do usunięcia");
			view_error.setHeaderText("Błąd!");
			view_error.setTitle("Okno błędu");
			view_error.showAndWait();
		}
	...
	// Metoda zatwierdzająca zmiany w pytaniu WPROWADZONE W TABELI I
	...
	// Metoda zapisująca pytanie po edycji
	@FXML
	...
	// Metoda anulująca wprowadzanie zmian/dodawanie pytania
	@FXML
	...
	void actionResultsAllGroups(ActionEvent event) throws SQLException {
		...
		lbl_result_table.setText("Średnie wyniki dla wszystkich grup");
		view_result_filter.setVisible(false);
		resultsList = FXCollections.observableArrayList();
	...
	void actionResultsAllUsers(ActionEvent event) throws SQLException {
		...
		view_result_filter.setVisible(false);
		lbl_result_table.setVisible(true);
		lbl_result_table.setText("Wyniki dla wszystkich użytkowników");
		resultsList = FXCollections.observableArrayList();
		ResultSet rs = conn.createStatement().executeQuery(
	...
	void actionResultsOneGroup(ActionEvent event) throws SQLException {
		...
		if (selectedItem != null) {
			lbl_result_table.setVisible(true);
			lbl_result_table.setText("Średnie wyniki dla grupy " + selectedItem.getUser().getGrupa());
	...
	void actionResultsOneUser(ActionEvent event) throws SQLException {
		...
		if (combo_result_user_credentials.getValue() != null) {
			...
			view_result_filter.setVisible(false);
			lbl_result_table.setText("Średnie wyniki dla " + imie + " " + nazwisko);
			combo_result_user_credentials.setValue(null);
		} else if (selectedItem != null) {
			flag = true;
			view_result_details.setVisible(true);
			view_result_filter.setVisible(false);
			lbl_result_table.setText("Średnie wyniki dla " + selectedItem.getUser().getImie() + " " + selectedItem.getUser().getNazwisko());
		...
		if (flag) {
			...
			// średnia liczba pytań / wszystkie
